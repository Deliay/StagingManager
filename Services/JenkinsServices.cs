...
namespace CheckStaging.Services
{
    ...
    {
        public const string JENKINS_MESSAGE_CHANNEL = "Jenkins";
        public const string JENKINS_INCOMING_CHANNEL = "staging";
        ...
        private JenkinsServices()
        {
            if (File.Exists(ConfigurationPath))
            {
                try
                {
                    ...
                    if (!RemindService.Instance.HasChannel(JENKINS_MESSAGE_CHANNEL))
                    {
        ...
        public string GetMainPanel(string owner)
        {
            ...
            Task.Run(() =>
            {
                ...
                RemindService.Instance.SendMessage(sb.ToString(), JENKINS_MESSAGE_CHANNEL);
            });
            return string.Empty;
        }
        ...
        public string Build(string owner, string staging, string branch)
        {
            ...
            Task.Run(() =>
            {
                ...
                if (BuildCache.Any(p => p.Value.building && ParameterCache[p.Key].staging == fullStagingName))
                {
                    ...
                    RemindService.Instance.SendMessage($"@{owner} 这个staging已经正在部署了，点此 [查看部署进度]({JenkinsConfiguration.BaseURL}job/{JenkinsConfiguration.Pipeline}/{build.Value.number})", JENKINS_MESSAGE_CHANNEL);
                    return;
                }
